...
class Generator(nn.Module):
    def __init__(self, input_dim, z_dim=100, hidden_dim=64):
        super(Generator, self).__init__()
        self.z_dim = z_dim 

        self.layers = nn.Sequential(
        # Input: (input_dim, 1, 1) -> Output: (hidden_dim * 4, 3, 3)
            nn.ConvTranspose2d(in_channels=input_dim, out_channels=hidden_dim * 4,
    ...
    def forward(self, noise_labels: torch.Tensor) -> torch.Tensor:
        x = noise_labels.unsqueeze(-1).unsqueeze(-1)
